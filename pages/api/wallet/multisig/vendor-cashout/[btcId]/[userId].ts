    ...
    if (method === 'POST') {
        ...
        form.parse(req, async function (err: any, fields: any, files: any) {
            ...
            try {
                ...
                var { sendToAddress, amount, vendorPassKey, mediatorPassKey } = fields;

                sendToAddress = sendToAddress[0];
                amount = amount[0];
                vendorPassKey = vendorPassKey[0];
                ...
                if (err || vendorPassKey) {
                    var vendorPrivateKey = await Wallet.findById(btcId)
                        .then(async (wallet: any) => {

                            if (!vendorPassKey || vendorPassKey !== wallet.pri[2]) {
                                return res.json({ msg: 'Invalid wallet account or vendor passkey!' });
                            }
                ...
                const buyerPrivateKey = await Wallet.findById(btcId)
                    .then((wallet: any) => wallet.pri[0])
                    .catch(() => res.json({ msg: 'Wallet account not found!' }));
                ...
                waitUntilUTXO(address)
                    .then(async data => {
                        ...
                        psbt.signInput(0, keyPairVendor);
